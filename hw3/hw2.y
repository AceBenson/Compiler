...
declaration_list
	...
	;
compound_statement
	: '{' '}' { char* buf=get_nbuf(3); strcpy(buf, "{}"); $$=buf; }
...
iteration_statement
	...
	;
jump_statement
	: CONTINUE ';' 			{ char* buf=get_nbuf(10); strcpy(buf, "continue;"); $$=buf; }
	| BREAK ';' 			{ char* buf=get_nbuf(7); strcpy(buf, "break;"); $$=buf; }
	| RETURN ';' 			{ char* buf=get_nbuf(8); strcpy(buf, "return;"); $$=buf; }
...
type_specifier
	...
	| CHAR 		{ char* buf=get_nbuf(5); strcpy(buf, "char"); $$=buf; }
	| SHORT 	{ char* buf=get_nbuf(6); strcpy(buf, "short"); $$=buf; }
	| INT 		{ char* buf=get_nbuf(4); strcpy(buf, "int"); $$=buf; }
	| LONG 		{ char* buf=get_nbuf(5); strcpy(buf, "long"); $$=buf; }
	| FLOAT 	{ char* buf=get_nbuf(6); strcpy(buf, "float"); $$=buf; }
	| DOUBLE 	{ char* buf=get_nbuf(7); strcpy(buf, "double"); $$=buf; }
	| SIGNED 	{ char* buf=get_nbuf(7); strcpy(buf, "signed"); $$=buf; }
	| UNSIGNED 	{ char* buf=get_nbuf(9); strcpy(buf, "unsigned"); $$=buf; }
	;
init_declarator_list
	: init_declarator { $$=$1; }
	| init_declarator_list ',' init_declarator { char* buf=get_buf(); sprintf(buf, "%s,%s", $1, $3); $$=buf; free($1); free($3); }
	;
init_declarator
	: declarator { $$=$1; }
	| declarator '=' initializer { char* buf=get_buf(); sprintf(buf, "%s=%s", $1, $3); $$=buf; free($1); free($3); }
	;
declarator
	: direct_declarator { $$=$1; }
	;
...
parameter_type_list
	: parameter_list { $$=$1; }
	| parameter_list ',' ELLIPSIS { char* buf=get_buf(); sprintf(buf, "%s,...", $1); $$=buf; free($1); }
	;
parameter_list
	: parameter_declaration { $$=$1; }
	| parameter_list ',' parameter_declaration { char* buf=get_buf(); sprintf(buf, "%s,%s", $1, $3); $$=buf; free($1); free($3); }
	;
parameter_declaration
	: declaration_specifiers declarator { char* buf=get_buf(); sprintf(buf, "%s%s", $1, $2); $$=buf; free($1); free($2); }
	| declaration_specifiers { $$=$1; }
	;
identifier_list
	: IDENTIFIER { $$=$1; }
	| identifier_list ',' IDENTIFIER { char* buf=get_buf(); sprintf(buf, "%s,%s", $1, $3); $$=buf; free($1); free($3); }
	;
initializer
	: assignment_expression { $$=$1; }
	| '{' initializer_list '}' { char* buf=get_buf(); sprintf(buf, "{%s}", $2); $$=buf; free($2); }
	| '{' initializer_list ',' '}' { char* buf=get_buf(); sprintf(buf, "{%s,}", $2); $$=buf; free($2); }
	;
initializer_list
	: initializer { $$=$1; }
	| initializer_list ',' initializer { char* buf=get_buf(); sprintf(buf, "%s,%s", $1, $3); $$=buf; free($1); free($3); }
	;
expression
	: assignment_expression { $$=$1; }
	| expression ',' assignment_expression { char* buf=get_buf(); sprintf(buf, "%s,%s", $1, $3); $$=buf; free($1); free($3); }
	;
...
assignment_operator
	: '=' 			{ char* buf=get_nbuf(2); strcpy(buf, "="); $$=buf;}
	| MUL_ASSIGN 	{ char* buf=get_nbuf(3); strcpy(buf, "*="); $$=buf; }
	| DIV_ASSIGN 	{ char* buf=get_nbuf(3); strcpy(buf, "/="); $$=buf; }
	| MOD_ASSIGN 	{ char* buf=get_nbuf(3); strcpy(buf, "%="); $$=buf; }
	| ADD_ASSIGN 	{ char* buf=get_nbuf(3); strcpy(buf, "+="); $$=buf; }
	| SUB_ASSIGN 	{ char* buf=get_nbuf(3); strcpy(buf, "-="); $$=buf; }
	;
conditional_expression
	: logical_or_expression { $$=$1; }
	| logical_or_expression '?' expression ':' conditional_expression { char* buf=get_buf(); sprintf(buf, "%s?%s:%s", $1, $3, $5); $$=buf; free($1); free($3); free($5); }
	;
logical_or_expression
	: logical_and_expression { $$=$1; }
	| logical_or_expression OR_OP logical_and_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>||<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
logical_and_expression
	: inclusive_or_expression { $$=$1; }
	| logical_and_expression AND_OP inclusive_or_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>&&<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
inclusive_or_expression
	: exclusive_or_expression { $$=$1; }
	| inclusive_or_expression '|' exclusive_or_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>|<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
exclusive_or_expression
	: and_expression { $$=$1; }
	| exclusive_or_expression '^' and_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>^<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
and_expression
	: equality_expression { $$=$1; }
	| and_expression '&' equality_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>&<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
equality_expression
	: relational_expression { $$=$1; }
	| equality_expression EQ_OP relational_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>==<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3);}
	| equality_expression NE_OP relational_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>!=<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
relational_expression
	: shift_expression { $$=$1; }
	| relational_expression '<' shift_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr><<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	| relational_expression '>' shift_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>><expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	| relational_expression LE_OP shift_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr><=<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	| relational_expression GE_OP shift_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>>=<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
shift_expression
	: additive_expression { $$=$1; }
	| shift_expression LEFT_OP additive_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr><<<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	| shift_expression RIGHT_OP additive_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>>><expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	;
additive_expression
	: additive_expression '+' multiplicative_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>+<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
	| additive_expression '-' multiplicative_expression { char* buf=get_buf(); sprintf(buf, "<expr>%s</expr>-<expr>%s</expr>", $1, $3); $$=buf; free($1); free($3); }
